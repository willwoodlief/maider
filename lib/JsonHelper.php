...
class JsonHelper {
    ...
    public static $b_output_headers = true;
    /**
     ...
     * @throws \Exception if cannot parse array to json
     */
     ...
     * @return string
     */
    public static function toStringAgnostic($what,$b_empty_is_null=true,$b_cast_bools_as_int = true) {
        if (is_null($what)) {return null;}
        if (is_string($what)) {
     ...
     * @param string|integer|float|null|boolean $var
     * @return bool
     */
    public static function var_to_boolean($var) {
     ...
     *
     */
    public static function dataFromString($data_type,$what) {
        if (is_null($what)) {return null;}
     ...
     * @throws \Exception if not a recognized $category, and if the data is not following the rules described in the param section
     * @return string <p>
     *  returns a string whose value depends on the data_type given above
     * </p>

     */
    public static function dataToString($data_type,$what) {
        //var_dump($data_type);
        // var_dump($what);
        switch ($data_type) {
            case 'boolean':
            ...
            case 'integer':
                ...
                if (is_float($what) || is_integer($what)) {
                    ...
                    throw  new \Exception("[$what] is not a float or an int or a string so cannot convert to integer");
                }
            case 'float':
                if (is_string($what)) {
                    $what = trim($what);
                ...
                if (is_float($what) || is_integer($what)) {
                    ...
                    throw  new \Exception("[$what] is not a float or an int or a string so cannot convert to float");
                }
            case 'text':
                if (is_array($what)) {
            ...
            case 'json':
                if (is_array($what) || is_null($what)) {
                    return JsonHelper::toString($what);
                } else {
                    throw  new \Exception("$data_type must use an array");
                }
            case 'date_time' :
                if (!$what) {
                    $what = time();
    ...
    public static function array_change_key_case_recursive(array $arr)
    {
     ...
     * </p>
     * @return string
     * @throws \Exception  if one of the operations fail
     */
    public static function tags_to_n($string,$replace_nobreak_space= false) {
        ...
        if ($replace_nobreak_space !== false) {
            ...
            if ($string === false) {
                throw new \Exception("Error in mb_ereg_replace in tags_to_n");
            }
        }
